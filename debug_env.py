...
try:
    ...
    if api_key:
        client = genai.Client(api_key=api_key, http_options=types.HttpOptions(client_args={"verify": certifi.where()}))
        print("Attempting generate_content...")
        response = client.models.generate_content(model="gemini-2.5-flash", contents="Diagnostics Test Message")
    ...
    print("FAILURE: Connection failed with error:")
    print(e)
