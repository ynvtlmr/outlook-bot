on run argv
	...
	tell application "Microsoft Outlook"
		try
			...
			-- Wait for draft to be fully created and window to open
			delay 1.5
			
			-- 2.1 Set Content - FIX ATTEMPT 3: Use plain text and let Outlook convert
			try
			    if responseBody is not "" then
			        -- Convert HTML <br> back to newlines for plain text
			        ...
			        -- FIX: Set as plain text first, then verify
			        set newContent to plainTextBody & return & return & oldContent
			        ...
			        -- If verification fails, try setting just our text
			        if verifyContent does not contain plainTextBody then
				    -- Last resort: set only our text (may lose original thread)
				    set content of newDraft to plainTextBody
				end if
			    end if
			on error e
