on run argv
	...
	tell application "Microsoft Outlook"
		try
			...
			-- 2.1 Set Content - FIX ATTEMPT 5: Don't close window, set content after longer delay
			try
			    if responseBody is not "" then
			        ...
			        -- Set content
			        set newContent to responseBody & "<br><br>" & oldContent
			        set content of newDraft to newContent
			        
			        -- Wait longer for Outlook to process
			        delay 2.0
				
				-- Verify and retry if needed
			        set verifyContent to content of newDraft
			        if verifyContent does not contain responseBody then
				    -- Retry: Set again after longer delay
				    delay 1.0
				    set content of newDraft to newContent
				    delay 1.0
				    
				    -- Final verification
				    set verifyContent2 to content of newDraft
				    if verifyContent2 does not contain responseBody then
				        -- Last attempt: Try setting just our text at the beginning
				        -- Find where the actual reply body starts (before signature)
				        -- Outlook reply format: [our reply] + signature + original thread
				        -- We'll try to insert at the very beginning
				        set content of newDraft to responseBody & "<br><br>" & oldContent
				        delay 1.5
				    end if
				end if
			    end if
