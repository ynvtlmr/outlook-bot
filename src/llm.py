...
class LLMService:
    ...
    def _discover_models(self):
        ...
        EXCLUDED_KEYWORDS = [
            'image', 'vision', 'audio', 'video', 'tts', 'speech', 'transcribe', 
            'whisper', 'dall-e', 'embedding', 'search', 'moderation', 'realtime',
            'creation', 'edit', '001', '002', # Legacy numeric versions sometimes imply specialized/old
            'preview', 'exp', 'codex', 'legacy'
        ]
        ...
        if self.gemini_client:
            try:
                for m in self.gemini_client.models.list():
                    ...
                    # 2. Exclude Experimental/Unstable (unless it's just a version suffix, but 'exp' usually implies beta)
    ...
    def _generate_openai(self, model_id, prompt):
        completion = self.openai_client.chat.completions.create(
            model=model_id,
            messages=[
                {"role": "user", "content": prompt} # System prompt is part of 'prompt' arg in legacy signature, but we can split if we want.
            ...
            try:
                ...
                if provider == 'gemini':
                    ...
                    completion = self.openai_client.chat.completions.create(
                        ...
                        # OpenAI's 'json_object' mode ensures valid JSON, but usually expects a root object { "data": [...] }
                        ...
                        # For now, let's try standard mode if we aren't sure about the structure, or strictly ask for an object.
                    )
