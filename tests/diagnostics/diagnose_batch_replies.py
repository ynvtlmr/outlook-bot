    ...
    try:
        ...
        if not service.gemini_client:
            ...
            if not gemini_key:
                ...
                print("[Root Cause] Client initialization failed despite API key being set")
        ...
        try:
            response = service.gemini_client.models.generate_content(
                model=gemini_model, contents=test_prompt, config={"response_mime_type": "application/json"}
            )
...
def test_scenario_3b_detailed_json():
    ...
    try:
        ...
        test_batch = [{"id": "test-123", "subject": "Test", "content": "Test content"}]
        ...
        prompt_batch = [
            {"id": item["id"], "subject": item["subject"], "content": item["content"]} for item in test_batch
        ]
        ...
        response = service.gemini_client.models.generate_content(
            model=gemini_model, contents=full_prompt, config={"response_mime_type": "application/json"}
        )
